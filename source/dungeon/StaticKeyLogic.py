...
def item_in_locations(state, item, player, locations):
    return any(item_name(state, name, loc_player) == (item, player) for name, loc_player in locations)


def set_static_key_rules(world, player):
    from Rules import set_rule, add_rule, forbid_item, set_always_allow
